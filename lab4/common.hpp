...
inline void osi_recv_log() {
    std::cout << "[Layer 4 - Transport] recv()" << std::endl;
}

inline void osi_deserialize_log(const Message& msg) {
    std::cout << "[Layer 6 - Presentation] deserialize Message -> "
              << message_type_to_string(msg.type) << std::endl;
}

inline void osi_session_log(const std::string& text) {
    std::cout << "[Layer 5 - Session] " << text << std::endl;
}

inline void osi_app_log(const std::string& text) {
    std::cout << "[Layer 7 - Application] " << text << std::endl;
}

inline void osi_prepare_send_log(const std::string& text) {
    std::cout << "[Layer 7 - Application] prepare response: " << text << std::endl;
}

inline void osi_serialize_log(uint8_t type) {
    std::cout << "[Layer 6 - Presentation] serialize Message -> "
              << message_type_to_string(type) << std::endl;
}

inline void osi_send_log() {
...
inline int send_message_raw(int sockfd, uint8_t type, const std::string& payload) {
    Message msg{};
...
inline int send_message(int sockfd, uint8_t type, const std::string& payload) {
    osi_prepare_send_log(message_type_to_string(type));
    osi_serialize_log(type);
    osi_send_log();
    return send_message_raw(sockfd, type, payload);
}
